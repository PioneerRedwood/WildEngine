...
int read_bitmap_header_info(FILE* fp, bitmap* bmp, int* offset) {
	...
	*offset += ret_code;
	...
	if (fpread(&h->size, sizeof(u_int32_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int32_t);

	if (fpread(&h->reserved1, sizeof(u_int16_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int16_t);

	if (fpread(&h->reserved2, sizeof(u_int16_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int16_t);

	if (fpread(&h->pixel_start_offset, sizeof(u_int32_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int32_t);
...
int read_bitmap_core_header(FILE* fp, bitmap* bmp, int* offset) {
	...
	if (fpread(&h->width, sizeof(u_int16_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int16_t);

	if (fpread(&h->height, sizeof(u_int16_t), *offset, fp) == 0) return 1;
	*offset += sizeof(u_int16_t);
	...
	*/
	...
	if (fpread(&h->bits_per_pixel, sizeof(u_int16_t), 1, *offset, fp) == 0) return 1;
	*offset += sizeof(u_int16_t);
...
int main(int argc, char** argv) {
	...
	if (fpread(&bmp.header_type, sizeof(u_int32_t), file_offset, fp) == 0) {
